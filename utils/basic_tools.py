...
def features_combination(features_all, cb_settings, target_size = 64):
    ...
    for name, ft_all in features_all.items():
        ...
        new_ft[name] = np.asarray([cv2.resize(ft, (target_size, target_size),
                             interpolation=cv2.INTER_LINEAR) for ft in ft_all])
...
def normalize(feature):
    ...
    result = (feature- np.min(feature,axis=0)) / (np.ptp(feature, axis=0) + 1e-6)
...
def sliding_window(image, size, func = None):
    ...
    if len(image.shape) == 2:
        img_pad = np.pad(image,((h1_pad,h2_pad),(w1_pad,w2_pad)), 'reflect')
    else:
        img_pad = np.pad(image, ((h1_pad, h2_pad), (w1_pad, w2_pad),(0,0)), 'reflect')
