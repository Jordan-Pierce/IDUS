...
class MyLoss(nn.Module):
    def __init__(self, pseudo_masks, n_classes=7, ignore_index=-1):
        ...
        counts = np.bincount(
            pseudo_masks[pseudo_masks != ignore_index].reshape(-1),
            minlength=n_classes)
...
def train_one_epoch(dataset, n_classes, max_epoch, save_path,
                    train_encoder_epoch=150):
    """
    ...
    for e in range(max_epoch):
        ...
        p_dict['loss'] = np.around(t_losses['loss'] / t_losses['batches'],
                                   decimals=3)
        p_dict['loss1'] = np.around(t_losses['loss1'] / t_losses['batches'],
                                    decimals=3)
        p_dict['loss2'] = np.around(t_losses['loss2'] / t_losses['batches'],
                                    decimals=3)
        ...
        if (e + 1) % 10 == 0:
            ...
            torch.save(net.state_dict(),
                       os.path.join(save_path, 'epoch_' + str(e + 1) + '.pth'))

    # update masks
    pseudo_masks = update_pseudo_mask(net,
                                      dataset.images(),
                                      n_classes,
                                      n_segments=256,
                                      compactness=1.0,
...
if __name__ == '__main__':
    ...
    # Iterate over the specified number of iterations, training the model
    # each time
    for i in range(max_iterations):
        ...
        dataset.pseudo_masks = train_one_epoch(dataset,
                                               n_classes,
                                               max_epoch,
                                               model_dir)
